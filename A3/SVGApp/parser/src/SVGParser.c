...
/* My helper HTML Functions */
char* fileNameRepeatition(char *file)
{
	char * string = calloc(4000, sizeof(char));
...
{
	char * string = fileNameRepeatition(filename);
	SVGimage * svg = createValidSVGimage(string, "svg.xsd");


	char* grouplist = groupListToJSON(svg->groups);
	char* pathlist = pathListToJSON(svg->paths);
	char* rectlist = rectListToJSON(svg->rectangles);
	char* circlelist = circListToJSON(svg->circles);
	char* attrlist = attrListToJSON(svg->otherAttributes);

	char* jsonObj = calloc(100000, sizeof(char));

	strcpy(jsonObj, "[");
	strcat(jsonObj, pathlist);
	strcat(jsonObj, ",");
	strcat(jsonObj, grouplist);
	strcat(jsonObj, ",");
	strcat(jsonObj, rectlist);
	strcat(jsonObj, ",");
	strcat(jsonObj, circlelist);
	strcat(jsonObj, ",");
	strcat(jsonObj, attrlist);
	strcat(jsonObj, "]");
...
char* getTitle (char *filename)
{
	...
	char* title = calloc(530, sizeof(char));
...
char* getDescription (char *filename)
{
	...
	char* description = calloc(530, sizeof(char));
	strcpy(description, svg->description);
...
char * requestCreateSVG(char * filename, char * title, char * description)
{
	...
	char * file = fileNameRepeatition(filename);
	printf("\n Create SVG IMAGE: \n");
	printf("File Name: %s\n Title: %s\n Description: %s", filename, title, description);

	SVGimage * svgdoc = create_svgdoc();
	//printf(svgdoc)
	strcpy(svgdoc->namespace,  "http://www.w3.org/2000/svg");
	strcpy(svgdoc->title,  title);
	strcpy(svgdoc->description,  description);


	bool writeSVG = writeSVGimage(svgdoc, file);
	if(writeSVG == false) return "Unable to write to file";

	char * svgToJSON = SVGtoJSON(svgdoc);

	deleteSVGimage(svgdoc);
	free(file);
...
void addRectangle(char* filename, char* shapeType, float xCor, float yCor, float rwCor, float hCor ,char* units)
{
	...
	char * file = fileNameRepeatition(filename);
	SVGimage* svg = createValidSVGimage(file, "svg.xsd");
	...
	if(strcmp((char*)shapeType, "Rectangle"))
	{
		...
		printList(svg->circles, "Circle");

		addComponent(svg, CIRC, cir);

		printf("After component Add: \n");
		printList(svg->circles, "Circle");
	}else
	{
		...
		printf("Before component: \n");
		printList(svg->rectangles, "Rectangle");
	...
	bool valid = writeSVGimage(svg, file);
...
void scaleShape(char* filename, char* shapeType, char* scaleFactor)
{
	...
	if(strcmp((char*)shapeType, "Rectangle") == 0)
	{

		List* rects = getRects(svg);
		printList(rects, "Rectangle");
